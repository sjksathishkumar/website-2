...
class SearchModel extends ModelBase
{
  	public function search( $data ) {
   	   ...
   	   if ( isset( $data ) )
		{
			...
			MATCH( P.post_details, P.video_title ) AGAINST( '".$data."' IN BOOLEAN MODE ) && U.mode = '1' && B.id IS NULL || 
			MATCH( P.post_details, P.video_title ) AGAINST( '".$data."*' IN BOOLEAN MODE ) && U.mode = '1' && B.id IS NULL
			GROUP BY P.id
			";
     ...
     public function searchUsers( $data, $session ) {
			...
			LEFT JOIN block_user B ON U.id = B.user && B.user_blocked = :id && B.status = '1'
			WHERE 
			U.username LIKE :query  && U.status = 'active' && U.mode = '1' && B.id IS NULL || 
			U.bio LIKE :query && U.status = 'active' && U.mode = '1' && B.id IS NULL || 
			U.name LIKE :query && U.status = 'active' && U.mode = '1' && B.id IS NULL
			GROUP BY U.id
			";
			$output = $this->db->prepare( $sql );
			$output->bindValue( ':query', '%'.$data.'%', PDO::PARAM_STR );
