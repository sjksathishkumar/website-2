...
<a class="btn btn-primary" style="float:right; margin-right:10px; margin-bottom:10px;" href="downcsv.php"><i class="icon-upload icon-white"></i>&nbsp;&nbsp;Download Report</a>
					...
					?>
                      <thead>
                          <tr>
							  ...
							  while($rrr = mysql_fetch_assoc($dequery))
							  {
								if($rrr['pf'] == 0) {?>
                              <th>PF</th>
                             <?php
        ...
        <td><?php echo round($row['val1']+$row['val2']+$row['val3']+$row['val4']+$row['val5']+$row['val6']+$row['val7']+$row['val8']-$row['val9']-$row['esval10']-$row['pt'],0,PHP_ROUND_HALF_UP);?></td>
